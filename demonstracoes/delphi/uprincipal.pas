...
//  ads.Campo('dhCont_B28').Value := FormatDateTime('YYYY-mm-dd"T"HH:mm:ss',now) + '-03:00';
...
begin
  mmXml.Text := spdNFCe.EnviarNF('0001',mmXml.Text);
...
procedure TfrmPrincipal.btnGerarDSClick(Sender: TObject);
  ...
  procedure DadosDoNFCe(aDs : TspdNFCeDataSets; serie : String);
  var
    _NRNota : String;
  begin
    aDs.campo('versao_A02').Value   := '3.10'; //Versão do leiaute
    aDs.Campo('cUF_B02').Value      := '41'; //Código da UF do emitente do Documento Fiscal
    aDs.Campo('cNF_B03').Value      := '12345678'; //Código Numérico que compõe a Chave de Acesso
    aDs.Campo('natOp_B04').Value    := 'VENDA MERC.ADQ.REC.TERC'; //Descrição da Natureza da Operação
    aDs.Campo('indPag_B05').Value   := '0'; //Indicador da forma de pagamento: 0 – pagamento à vista; 1 – pagamento à prazo; 2 - outros.
    aDs.Campo('mod_B06').Value      := '65'; //Modelo do Documento Fiscal
    aDs.Campo('serie_B07').Value    := serie; //Série do Documento Fiscal
    ...
    aDs.Campo('cMunFG_B12').Value   := '4115200'; //Codigo Maringa
    aDs.Campo('tpImp_B21').Value    := '4';
	{Formato do DANFE: 0-Sem geração de DANFE; 1-DANFE normal , Retrato; 2-DANFE normal, Paisagem; 3-DANFE Simplificado;
	 4-DANFE NFC-e; 5-DANFE NFC-e em mensagem eletrônica. Nota: O envio de mensagem eletrônica pode ser feita de forma simultânea
   com a impressão do DANFE. Usar o tpImp=5 quando esta for a única.}
    aDs.Campo('tpEmis_B22').Value   := '1';
...
//    ads.Campo('dhCont_B28').Value := FormatDateTime('YYYY-mm-dd"T"HH:mm:ss',now) + '-03:00';
  ...
  procedure DadosDoItem(aDs : TspdNFCeDataSets);
  Var
   i : Integer;
  begin
    for i := 1 to 1 do
    ...
    aDs.Campo('nItem_H02').Value    := IntToStr(i); //Número do item
    aDs.Campo('cProd_I02').Value    := '927803'; //Código do produto ou serviço
    aDs.Campo('cEAN_I03').Value     := ''; //GTIN (Global Trade Item Number) do produto, antigo código EAN ou código de barras
    aDs.Campo('xProd_I04').Value    := 'NOTA FISCAL EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL'; //Descrição do produto ou serviço
    ads.Campo('NCM_I05').Value      := '30043929'; //Código NCM com 8 dígitos ou 2 dígitos (gênero)
    aDs.Campo('CFOP_I08').Value     := '5101'; //Código Fiscal de Operações e Prestações
    aDs.Campo('uCom_I09').Value     := 'UN'; //Unidade Comercial
    ...
    aDs.Campo('vUnCom_I10a').Value  := '1.00'; //Valor Unitário de Comercialização
    aDs.Campo('vProd_I11').Value    := '1.00'; //Valor Total Bruto dos Produtos ou Serviços
    aDs.Campo('cEANTrib_I12').Value := ''; //GTIN (Global Trade Item Number) da unidade tributável, antigo código EAN ou código de barras
    aDs.Campo('uTrib_I13').Value    := 'UN'; //Unidade Tributável
    aDs.Campo('qTrib_I14').Value    := '1.00'; //Quantidade Tributável
    aDs.Campo('vUnTrib_I14a').Value := '1.00'; //Valor Unitário de tributação
    aDs.Campo('indTot_I17b').Value  := '1'; //Indica se valor do Item (vProd) entra no valor total da NF-e (vProd)
  ...
  Procedure DadosTotalizadores(aDs : TspdNFCeDataSets);
  begin
    aDs.Campo('vBC_W03').Value := '0.00'; //Base de Cálculo do ICMS
    aDs.Campo('vICMS_W04').Value := '0.00'; //Valor Total do ICMS
    ads.Campo('vICMSDeson_W04a').value := '0.00';
  ...
  procedure DadosPagamento(aDs : TspdNFCeDataSets);
  begin
    aDs.IncluirPart('YA');
    aDs.Campo('tPag_YA02').Value := '01'; //Forma de pagamento: 01- Dinheiro; 02 -Cheque; 03- Cartão de Crédito; 04- Cartão de Débito; 05- Crédito Loja;
										  //10- Vale Alimentação; 11- Vale Refeição; 12- Vale Presente; 13- Vale Combustível; 99 – Outros.
    aDs.Campo('vPag_YA03').Value := '1.00'; //Valor do Pagamento
    {
    aDs.Campo('CNPJ_YA05').Value := '48336647000115';
    aDs.Campo('tBand_YA06').Value := '01';
    ads.Campo('cAut_YA07').Value := '012121212';
    }
    aDs.SalvarPart('YA');
    {
    aDs.IncluirPart('YA');
    aDs.Campo('tPag_YA02').Value := '03'; //Forma de pagamento: 01- Dinheiro; 02 -Cheque; 03- Cartão de Crédito; 04- Cartão de Débito; 05- Crédito Loja;
										  //10- Vale Alimentação; 11- Vale Refeição; 12- Vale Presente; 13- Vale Combustível; 99 – Outros.
    aDs.Campo('vPag_YA03').Value := '48.90'; //Valor do Pagamento
    aDs.Campo('CNPJ_YA05').Value := '48336647000999';
    aDs.Campo('tBand_YA06').Value := '02';
    ads.Campo('cAut_YA07').Value := '999815414';
    aDs.SalvarPart('YA');
    }
    ads.Campo('infCpl_Z03').Value := 'VEND:LUB-REG.1                      Pre-venda: 14823|DOC.EMITIDO POR EPP OPTANTE PELO SIMPLES NACIONL PRAZO MAXIMO P/DEV.07 DIAS E C/PREVIA AUTORIZAC |';
    ads.Campo('infAdFisco_Z02').Value := 'CERT.NEGATIVA Nr.: 0012143331   Cod.autenticidade: TT9BATL2TBBAT2AL    |ICMS RECOLHIDO ANTECIPADAMENTE POR SUBST. TRIB. CFE ART. 47 E/OU 50  DO ANEXO VIII E ART. 6 DO ANEXO XIV DO RICMS-MT';
...
begin
    ...
    serie := InputBox('NF-e', 'Insira o número da Série:','1');
    spdNFCeDataSets.VersaoEsquema := pl_008i1;


    _Dir:='C:\Program Files\TecnoSpeed\NFCe\Arquivos\Templates\vm50b\Conversor\NFCeDataSets.xml';
    spdNFCeDataSets.XMLDicionario := _Dir;
    ...
    spdNFCeDataSets.Incluir;
    DadosDoNFCe(spdNFCeDataSets ,serie);
    DadosDoEmitente(spdNFCeDataSets);
    DadosDoDestinatario(spdNFCeDataSets);
    DadosDoItem(spdNFCeDataSets);
    DadosTotalizadores(spdNFCeDataSets);
    DadosPagamento(spdNFCeDataSets);

    spdNFCeDataSets.Salvar;

    mmXML.Text := spdNFCeDataSets.LoteNFCe.GetText;
    edtId.Text := obterNroResultado(mmXml.Text,'Id="NF','" versao="3.10"><ide');
end;

procedure TfrmPrincipal.btnGerartx2Click(Sender: TObject);
begin
  dlgTx2.InitialDir := ExtractFilePath(ParamStr(0));
  dlgTx2.Execute;
  if dlgTx2.FileName <> '' then
    mmXml.Text := spdNFCe.ConverterLoteParaXml(dlgTx2.FileName, lkTXTDataSet, pl_008i1);
